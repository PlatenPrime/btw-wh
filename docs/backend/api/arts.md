...
Частичное обновление артикула по идентификатору: поля **`limit`** и/или **`prodName`**. Узкий эндпоинт `PATCH /api/arts/:id/limit` по-прежнему обновляет только лимит; здесь можно изменить оба поля за один запрос или только одно из них.

**Доступ:** checkAuth + checkRoles(ADMIN).

**Запрос:** path `id` — MongoDB ObjectId. Body: `{ limit?: number, prodName?: string }` — должно быть указано **хотя бы одно** поле; `limit` неотрицательный, `prodName` — строка (в т.ч. пустая для сброса значения в БД).

**Ответ 200:** обновлённый документ Art (в ответе присутствует `prodName` среди выбранных полей).

**Ошибки:** 400 (валидация, отсутствуют оба поля, отрицательный `limit` и т.д.), 401, 403, 404 (артикул не найден), 500.
