...
**Зачистка промоутнутых из «Новинок»:** **POST `/api/skugrs/purge-promoted-from-newsku`** (роль ADMIN) одним вызовом обходит все группы с `prodName: newsku` и убирает из их массивов `skus` ссылки на карточки, у которых уже другой `prodName`. Документы `Sku` не удаляются и не меняются; в группах остаются только ещё не присвоенные (`prodName: newsku`). Счётчики ответа — в [API Skugrs](../api/skugrs.md).
