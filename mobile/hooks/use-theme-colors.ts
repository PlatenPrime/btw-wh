...
export function useThemeColors() {
  ...
  const getColorFromToken = (tokenPath: string): string => {
  ...
  return {
    ...
    card: {
      bg: getColorFromToken('card.bg'),
      border: getColorFromToken('card.border'),
    },
    // Цвета для диалогов и модальных окон (из токенов: background-0, outline-200)
    dialog: {
      bg: getColorFromToken('dialog.bg'),
      border: getColorFromToken('dialog.border'),
    },
 ...
 * @returns RGBA строку
 */
export function getThemeColorWithOpacity(colorHex: string, opacity: number): string {
