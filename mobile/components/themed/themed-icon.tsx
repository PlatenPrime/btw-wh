...
export type ThemedIconProps = VariantProps<typeof iconStyle> & {
  className?: string;
  family?: IconFamily;
  name?: string;
  color?: string;
...
const ThemedIcon = React.forwardRef<React.ComponentRef<typeof Svg>, ThemedIconProps>(
  function ThemedIcon(
    { size = "md", className, family, name, color, lightColor, darkColor, children, ...props },
    ref
  ) {
...
const createIcon = ({ viewBox = "0 0 24 24", path, d }: CreateIconProps) => {
  const IconComponent = React.forwardRef<
    ...
    Omit<ThemedIconProps, "viewBox" | "children" | "lightColor" | "darkColor"> & { className?: string }
